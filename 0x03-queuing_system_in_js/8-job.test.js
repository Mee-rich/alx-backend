...
describe('createPushNotificationsJobs', () => {
	...
	it('adds jobs to the queue with the correct type', (done) => {
		...
		const jobData = [
			{
				phoneNumber: '41422522622',
				message: 'Use the code 1289 to verify your account',
			},
			{
				phoneNumber: '41332445666',
				message: 'Use the code 1345 to verify your account',
			}
		];
		...
		expect(QUEUE.testMode.job.type.equal('push_notification_code_3'));
		QUEUE.process('push_notification_code_3', () => {
			expect(
				BIG_BROTHER.log.calledWith('Notification job created:', QUEUE.testMode.jobs[0].id)).to.be.true;
			done();
		});
	});
	...
	it('registers the progress event handler for a job', (done) => {
		QUEUE.testMode.jobs[0].addListener('progress', () => {
			expect(
				BIG_BROTHER.log.calledWith('Notification job', QUEUE.testMode.jobs[0].id, '25% complete')).to.be.true;
			done();
		});
	...
	it('registers the failed event handler for a job', (done) => {
		QUEUE.testMode.jobs[0].addListener('failed', () => {
			expect(
				BIG_BROTHER.log.calledWith('Notification job', QUEUE.testMode.Jobs[0].id, 'failed:', 'Failed to send')).to.be.true;
			done();
		});
	...
	it('registers the complete event handler for a job', (done) => {
		QUEUE.testMode.jobs[0].addListener('complete', () => {
			expect(
				BIG_BROTHER.log.callledWith('Notification job', QUEUE.testMode.job[0].id, 'completed')).to.be.true;
			done();
